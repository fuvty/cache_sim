...
unsigned int bit_tag_inv_mask = 0;   //How many bits of the tag.
short unsigned int bit_set = 0;   //How many bits of the set.
unsigned int bit_set_mask = 0;   //How many bits of the set.
/******************************************/
...
bool GetHitNum(char *address, uint32_t strtoulNum, bool *is_hit)
{
    bool is_store = false;
    bool is_load = false;
    bool is_space = false;
    ...
    temp = strtoulNum;
    // temp = strtoul(address + 2, NULL, 16) >> 1 << 1;
...
bool IsHit(bitset<32> flags)
{
    ...
    else if (t_assoc == set_associative)
    {
        ...
        auto currrent_set_direct = (flags.to_ulong()  >> bit_block) & bit_set_mask;
        // bitset<32> flags_set;
        ...
        current_set = currrent_set_direct;
        ...
        for (temp = (current_set * i_cache_set); temp < ((current_set + 1) * i_cache_set); temp++)
        {
            if (cache_item[temp][30] == true) //判断hit位是否为真
            {
                ...
                uint32_t mask_a = flags.to_ulong();
                uint32_t mask_b =  cache_item[temp].to_ulong() << (31-28);

                ret = (mask_a & bit_tag_inv_mask) == (mask_b & bit_tag_inv_mask);
            }
...
void GetRead(bitset<32> flags)
{
    ...
    else if (t_assoc == set_associative)
    {
        ...
        if (space == true)
        {
            ...
            uint32_t current_item_int = cache_item[current_line].to_ulong();
            auto toWrite = flags.to_ulong() >> (31 - 28);
            auto blendMask = bit_tag_inv_mask >> (31 - 28);
            current_item_int = (current_item_int & (~ blendMask)) | (toWrite & blendMask);
            cache_item[current_line] = current_item_int;
